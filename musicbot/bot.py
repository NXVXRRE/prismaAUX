...
class MusicBot(discord.Client):
    ...
    async def _autojoin_channels(self, channels):
        ...
        for channel in channels:
            if channel.server in joined_servers:
                print("Already joined registered channel in %s, skipping" % channel.server.name)
                continue

            if channel and channel.type == discord.ChannelType.voice:
                self.safe_print("Attempting to join %s" % (channel.server.name))
                ...
                try:
                    ...
                    if self.config.debug_mode:
                        traceback.print_exc()
                    print("Failed to join", channel.server.name)
    ...
    async def on_ready(self):
        ...
        print('\rConnected! prismaAUX v%s\n' % BOTVERSION)
        ...
        if self.config.bound_channels:
            ...
            print("No text channels are registered.")
        ...
        if self.config.autojoin_channels:
            ...
            print("   Registered voice channels")
            print("¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯")
            [self.safe_print(' • %s/%s' % (ch.server.name.strip(), ch.name.strip())) for ch in chlist if ch]

            if invalids and self.config.debug_mode:
                print("   Error joining voice channels")
                print("¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯")
                [self.safe_print(' • %s/%s' % (ch.server.name.strip(), ch.name.strip())) for ch in invalids if ch]
            ...
            print("Not registered on any voice channels.")
        ...
        self.safe_print("  Command identifier • '{}'".format(self.config.command_prefix))
        ...
        print("  Join owner • " + ['Disabled', 'Enabled'][self.config.auto_summon])
        print("  AUX mode • " + ['Disabled', 'Enabled'][self.config.auto_playlist])
        print("  Autopause • " + ['Disabled', 'Enabled'][self.config.auto_pause])
        print("  Delete Bot Responses • " + ['Disabled', 'Enabled'][self.config.delete_messages])
        if self.config.delete_messages:
            print("  Delete User Invokes • " + ['Disabled', 'Enabled'][self.config.delete_invoking])
        ...
        if self.config.autojoin_channels:
            ...
            print("Attempting to join owner...", flush=True)
            ...
            if owner_vc:
                ...
                print("Owner not found in a voice channel, could not join.")
    ...
    async def cmd_prismainvoke(self, channel, command=None):
        """
        Usage:
        -prismainvoke "<prismaNOW_command>"

        Will only work if prismaNOW is present in the Discord server.
        Any spaces must be replaced with underscores.
        """
        invoking_context = command.replace("+", " ")
        await self.safe_send_message(channel, content=invoking_context)
    ...
    @owner_only
    async def cmd_say(self, msg=None):
        """
        Usage:
        -say message
    ...
    async def cmd_play(self):
        ...
        Depreciated function. If you haven't yet,
        please use prismaNOW to listen to songs On-Demand.
        """
        dep_msg = "Depreciated function. Please use PrismaNOW. http://prismanow.us.to/"

        return Response(dep_msg, delete_after=10)
        time.sleep(20)
    ...
    async def old_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            try:
                ...
                return await self.old_play(player, channel, author, permissions, leftover_args, e.use_url)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        """.format(self.config.command_prefix, self.config.command_prefix)
        ...
        for e in info['entries']:
            ...
            if response_message.content.lower().startswith('y'):
                ...
                await self.old_play(player, channel, author, permissions, [], e['webpage_url'])
